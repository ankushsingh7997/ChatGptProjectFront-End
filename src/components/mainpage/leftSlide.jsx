...
function LeftSlide() {
  ...
  useEffect(() => {
    ...
    (async () => {
      await fetch(`https://summer-code-project-backend.vercel.app/fetchuser/${userid}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  async function handleDelete(uniqueKey) {
    let result = await fetch(
      `https://summer-code-project-backend.vercel.app/deleteChat/${userid}/${uniqueKey}`,
      {
        method: "DELETE",
        headers: {
  ...
  const handleDeleteConversation = async () => {
    if (userData.length != 0) {
      let result = await fetch(
        `https://summer-code-project-backend.vercel.app/deleteAllChat/${userid}`,
        {
          method: "DELETE",
          headers: {
            "content-Type": "application/json",
